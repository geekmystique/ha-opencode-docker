...
// Standalone deployments have no Home Assistant Ingress in front of this
// port, so this proxy is the only thing that can require a credential.
// Both must be set (non-empty) to turn auth on; otherwise the port is open
// to anyone who can reach it, matching this project's existing LAN-mode
// precedent (documented, not silently insecure).
const WEB_USERNAME = process.env.WEB_USERNAME || "";
const WEB_PASSWORD = process.env.WEB_PASSWORD || "";
const AUTH_ENABLED = Boolean(WEB_USERNAME && WEB_PASSWORD);
...
function isAuthorized(req) {
  ...
  return safeEqual(decoded.slice(0, sep), WEB_USERNAME) && safeEqual(decoded.slice(sep + 1), WEB_PASSWORD);
}
