...
# Standalone: there is no Supervisor to auto-inject SUPERVISOR_TOKEN. HA_TOKEN
# (used by zigporter) is already set by the parent service script from
# HA_ACCESS_TOKEN; only warn here, and never clobber it with an empty value.
if [ -z "${HA_ACCESS_TOKEN:-}" ] && [ -z "${SUPERVISOR_TOKEN:-}" ]; then
    echo "Warning: HA_ACCESS_TOKEN not set. MCP integration may not work."
fi
