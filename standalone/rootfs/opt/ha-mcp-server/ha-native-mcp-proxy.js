...
const SUPERVISOR_API = process.env.HA_NATIVE_MCP_BASE_URL || process.env.HA_API_BASE_URL || "http://supervisor/core/api";
// Standalone (non-Supervisor) deployments only set HA_ACCESS_TOKEN.
const SUPERVISOR_TOKEN = process.env.SUPERVISOR_TOKEN || process.env.HA_ACCESS_TOKEN;
...
if (!SUPERVISOR_TOKEN) {
  log("error", "SUPERVISOR_TOKEN or HA_ACCESS_TOKEN is required");
  process.exit(1);
}
