...
// tools (backups, add-on/store management, host/OS/network health, jobs) are
// reachable at all - none of those have a Home Assistant Core equivalent.
const HAS_SUPERVISOR = Boolean(process.env.SUPERVISOR_TOKEN);
// Long-lived token for direct HA Core calls. Every other REST/WebSocket call
// in this file that used to require SUPERVISOR_TOKEN falls back to this, so a
// standalone container with only HA_ACCESS_TOKEN set (and HA_API_BASE_URL
...
if (!SUPERVISOR_TOKEN) {
  console.error("Error: SUPERVISOR_TOKEN or HA_ACCESS_TOKEN environment variable is required");
  process.exit(1);
}

const SUPERVISOR_UNAVAILABLE_ERROR = "This tool requires Home Assistant Supervisor.\n\n" +
  "It is not available in this standalone Docker deployment: Supervisor manages backups, " +
  "add-ons/apps, and host/OS updates, and none of that exists outside Home Assistant OS or " +
  "a Supervised install. Config editing, validation, entity/service tools, and the rest of " +
  "this MCP server work normally - only Supervisor-specific management is unavailable here.";
  ...
  try {
    ...
    switch (name) {
      ...
      case "get_supervisor_health": {
        if (!HAS_SUPERVISOR) {
          return makeCompatibleResponse({
            content: [createTextContent(SUPERVISOR_UNAVAILABLE_ERROR, { audience: ["user"], priority: 1.0 })],
            isError: true,
          });
        }
        ...
        const result = await new Promise((resolvePromise, rejectPromise) => {
          const proc = execFile("/usr/local/bin/hab", cleanArgs, {
            ...
            env: {
              ...
              HAB_URL: process.env.HA_URL || "http://supervisor/core",
