...
const SUPERVISOR_CORE_API = process.env.HA_API_BASE_URL || "http://supervisor/core/api";
const SUPERVISOR_API = "http://supervisor";
// Standalone deployments have no Supervisor-proxied websocket, so derive the
// real Core websocket URL from the same base URL used for REST calls.
function deriveWebsocketUrl(coreApiUrl) {
  try {
    const url = new URL(coreApiUrl);
    url.protocol = url.protocol === "https:" ? "wss:" : "ws:";
    url.pathname = url.pathname.replace(/\/api\/?$/, "") + "/api/websocket";
    return url.toString();
  } catch {
    return "ws://supervisor/core/websocket";
  }
}
const SUPERVISOR_WEBSOCKET = process.env.HA_WS_URL || deriveWebsocketUrl(SUPERVISOR_CORE_API);
